...
档位口径（与生产 3 档的关系，P1-#11 决策：两套口径各自成立，此处注明映射）：
  - 本策略用 4 档 ORDER = [加仓, 观望, 减仓, 清仓]，是**测量仪器**：
    「清仓」= 评分最深的空头档，用于单独度量强空头信号的质量——
    生产 TH_SELL=-8 的标定正是依赖该档的分布与胜率数据。
  - 生产 rule_engine 是 3 档 [加仓, 减仓, 观望]，是**产品决策**
    （P0-5 拍板：绝不输出模糊表述）。
  - 映射：回测「清仓」档 ≈ 生产「减仓」档的强信号子集；
    回测「减仓」档 ≈ 生产「减仓」档的全部（含浅空头）。
    因此回测中「加仓 vs 清仓」胜率差衡量的是生产减仓信号里最强部分的区分度，
    不可与生产三档统计直接混用。本策略已 import 生产 FACTOR_WEIGHTS/TH_BUY/TH_SELL，
    评分与权重同源；仅档位切分深度不同。
"""
