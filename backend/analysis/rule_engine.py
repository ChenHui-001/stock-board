...
# ------------------------------------------------------------------ 评分权重与阈值（V2）
# 依据 backend/backtest 因子级归因（2026-09-01）：10 只 A 股面板、7050 个事件、
# 2023-09 ~ 2026-08，信号日收盘计算 → 次日开盘买入 → 持有 5 日收盘卖出。
#
# 关键实证结论：
#   - 旧版总分 IC 在 7 个半年期里 **0/7 为正**（均值 -0.068），即评分越高、
#     后续 5 日表现越差，方向高度一致
#   - 趋势跟随类因子稳定为负：斜率 0/7、20 日涨跌 0/7、均线排列 1/7、突破 1/7
#   - 唯一 7/7 稳定为正的是「MA20 乖离修正」（IC +0.074，IR 1.54 最高）
#   - 站上均线条数 3/7、盘口 4/7、基本面 2/7 → IC 符号反复翻转，属噪音
#
# V2 采取「降权」而非「反向」：样本外检验证明把负 IC 因子反向同样无效
# （严格样本外 t=-1.13），反向只是另一种形式的过拟合；对不确定性诚实的
# 处理是缩小其影响，而不是赌一个方向。
#
# 注：这份权重同时被 backend/backtest/score_strategy.py 引用，保证回测与
# 生产口径一致，避免两边漂移。
FACTOR_WEIGHTS: dict[str, float] = {
    "above": 0.0,          # 站上均线条数：IC 3/7 符号翻转 → 剔除（原 ±16）
    "arrange": 1 / 3,      # 均线排列：IC 1/7     → ±18 → ±6
    "slope": 5 / 14,       # 四均线斜率：IC 0/7   → ±14 → ±5
    "chg20": 1 / 3,        # 20 日涨跌：IC 0/7（最负 -0.095）→ ±6 → ±2
    "sr_pos": 3 / 8,       # 支撑压力突破：IC 1/7 → +8  → +3
    "sr_neg": 1 / 3,       # 支撑压力跌破：IC 1/7 → -12 → -4
    "dev": 3.0,            # MA20 乖离修正：唯一 7/7 稳定为正 → ±4 → ±12
    "intraday": 0.5,       # 盘口信号：IC 4/7     → clamp ±8 → ±4
    "fundamental": 0.5,    # 基本面：IC 2/7       → ±8 → ±4
}

# 决策阈值：V2 分值量级约为旧版的 1/2.5，按分位数重新标定。
# 目标分布 加仓 ~14% / 观望 ~34% / 减仓 ~27% / 清仓 ~25%，
# 消除旧版两极分化（旧版观望仅 16.6%、清仓 34.0%）。
TH_ADD = 10.0      # >= 加仓
TH_HOLD = 0.0      # >= 观望
TH_REDUCE = -8.0   # >= 减仓，否则清仓

# 置信度区间：旧版 45~92。回测已证明评分方向性有限（IC 0/7 为正），
# 高分不应再对应「高置信度」，故收敛到 35~78，避免用高置信度驱动加仓。
